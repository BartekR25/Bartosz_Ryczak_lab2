...
def compare_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    """
    if x1.shape != y1.shape or min(x1.shape) == None:
        return None
    if  x2.shape != y2.shape or min(x2.shape) == None:
        return None

    fig, ax = plt.subplots()
    ax.plot(x1, y1, "b", label = label1, linewidth = 4)
    ax.plot(x2, y2, "k", label = label2, linewidth = 2)
...
def parallel_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    if min(x1.shape) == None or  x1.shape != y1.shape or len(x1) == 1:
        return None
    if min(x2.shape) == None or x2.shape != y2.shape or len(x2) == 1:
        return None
